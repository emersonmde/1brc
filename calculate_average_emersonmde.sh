...
#  echo "Young Generation Size: $YOUNG_GEN_SIZE"
  echo "Garbage Collector Type: $GC_TYPE"

#  JAVA_OPTS="--enable-preview -Xms$MIN_HEAP_SIZE -Xmx$MAX_HEAP_SIZE -Xmn$YOUNG_GEN_SIZE -XX:ParallelGCThreads=$num_cores -XX:ConcGCThreads=$(echo "$num_cores / 2" | bc) $GC_TYPE"
